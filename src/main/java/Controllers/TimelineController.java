...
public class TimelineController implements Initializable {
    ...
    private int calculateCurrentPage(double scrolledLength) {
        if(timelineType.equals("absolute")==false) {
            ...
            if(pageTimeUnit==Calendar.DAY_OF_MONTH) {
                ...
                return (int) numbDaysScrolled / 30;
    ...
    private void moveTimelineBackground(ScrollDirection direction){
        if(generatingBackground==false) {
            ...
            if(currentPageNumber >1 && direction==ScrollDirection.RIGHT) {
            ...
            if(timelineType.equals("absolute")) {
                ...
                newSpacerWidth = intPageWidth * (pagenumb-1);
            }
    ...
    private GridPane createTimelineBackgroundPage(GridPane pageToRepurpose, int page) {
        ...
        if(pageToRepurpose==null || timelineType.equals("absolute")==false) {
        ...
        if(pageToRepurpose==null || pageTimeUnit==Calendar.WEEK_OF_MONTH || pageTimeUnit==Calendar.MONTH || timelineType.equals("absolute")==false) {
    ...
    private void loadEvents(int currentPageNumber, ScrollDirection direction) {
        if(busy == false && (direction==ScrollDirection.NONE || (direction==ScrollDirection.RIGHT && currentPageNumber > lastPageLoadedEvents))) {
            ...
            if(timelineType.equals("absolute") || timelineType.equals("semi")) {
                from.setTime(timeline.getStartDate());
                //to.setTime(timeline.getStartDate());
                if(timelineType.equals("semi")) {
                    int semiUnit = 0;
                    if(timeline.getTimeUnit().equals("second") || timeline.getTimeUnit().equals("Seconds")) {
                        semiUnit = Calendar.SECOND;
                    } else if(timeline.getTimeUnit().equals("minute") || timeline.getTimeUnit().equals("Minutes")) {
                        semiUnit = Calendar.MINUTE;
                    } else if(timeline.getTimeUnit().equals("year") || timeline.getTimeUnit().equals("Years") || timeline.getTimeUnit().equals("decade") || timeline.getTimeUnit().equals("Decades") || timeline.getTimeUnit().equals("century") || timeline.getTimeUnit().equals("Centuries")) {
                        semiUnit = Calendar.YEAR;
                    }
                    if (direction == ScrollDirection.RIGHT) {
                        //load events of the next day
                        from.add(semiUnit, (currentPageNumber + 3) * intPerPage);
                        to.add(semiUnit, (currentPageNumber + 4) * intPerPage);
                    } else if (direction == ScrollDirection.NONE) {
                        //when the timeline is loaded
                        to.add(semiUnit, 4 * intPerPage);
                    } else {
                        //left edge
                        from.add(semiUnit, currentPageNumber * intPerPage);
                        to.add(semiUnit, (currentPageNumber + 1) * intPerPage);
                    }
                } else {
                    if (direction == ScrollDirection.RIGHT) {
                        //load events of the next day
                        from.add(pageTimeUnit, currentPageNumber + 3);
                        to.add(pageTimeUnit, currentPageNumber + 4);
                    } else if (direction == ScrollDirection.NONE) {
                        //when the timeline is loaded
                        to.add(pageTimeUnit, 4);
                    } else {
                        //left edge
                        from.add(pageTimeUnit, currentPageNumber);
                        to.add(pageTimeUnit, currentPageNumber + 1);
                    }
                }
            } else {
                if (direction == ScrollDirection.RIGHT) {
                    //load events of the next day
                    fromInt = (currentPageNumber + 3) * intPerPage;
                    toInt= (currentPageNumber + 4) * intPerPage;
                } else if (direction == ScrollDirection.NONE) {
                    //when the timeline is loaded
                    toInt = 4 * intPerPage;
                } else {
                    //left edge
                    fromInt = currentPageNumber * intPerPage;
                    toInt=(currentPageNumber + 1) * intPerPage;
                }
            }

            TimelineController timelineController = this;
            int finalFromInt = fromInt;
            int finalToInt = toInt;
                ...
                public ArrayList<EventThumbnail> call() {
                    ...
                    try {
                        ...
                        if(timelineType.equals("relative")) {
                            resEvent = timelineController.getTimeline().getEventsBetween(finalFromInt, finalToInt);
                        } else {
                            resEvent = timelineController.getTimeline().getEventsBetween(calendarToLocalDateTime(from), calendarToLocalDateTime(to));
                        }
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        timelineBackgroundHolder.widthProperty().addListener((obs, oldVal, newVal) -> {
            ...
            this.timelineWidth = (double) newVal;
        });
    }
